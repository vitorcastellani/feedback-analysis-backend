...
def test_create_feedback(client, db_session):
    ...
    response = client.post(
        "/api/feedback",
        json={"message": "Great service!", "campaign_id": campaign.id}
    )
...
def test_create_feedback_inactive_campaign(client, db_session):
    ...
    response = client.post(
        "/api/feedback",
        json={"message": "Won't work", "campaign_id": campaign.id}
    )
...
def test_create_feedback_max_answers_exceeded(client, db_session):
    ...
    first_response = client.post(
        "/api/feedback",
        json={"message": "First answer", "campaign_id": campaign.id}
    )
    assert first_response.status_code == 201

    second_response = client.post(
        "/api/feedback",
        json={"message": "Second answer", "campaign_id": campaign.id}
    )
...
def test_create_feedback_multiple_answers_not_allowed(client, db_session):
    ...
    first_response = client.post(
        "/api/feedback",
        json={"message": "First feedback", "campaign_id": campaign.id}
    )
    assert first_response.status_code == 201

    second_response = client.post(
        "/api/feedback",
        json={"message": "Second feedback", "campaign_id": campaign.id}
    )
    assert second_response.status_code == 400
    data = second_response.get_json()
